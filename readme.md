...
`pixiv推荐作品` : 从pixiv.net主页的推荐作品中随机挑选一张回复给指令发送者

## 详细使用指南

在koishi插件市场找到puppeteer插件，设置该插件args:

1. `--no-sandbox`
2. `--disk-cache-dir=D:\示例路径`
3. `--user-data-dir=D:\示例路径`
4. `--enable-features=PersistentStorage`
5. `--disable-session-storage`
6. `--disable-browser-side-navigation`
7. `--no-first-run`
8. `--enable-cookies`
9. `--disable-cookie-encryption`
10. `--disable-web-security` 注意该选项有很高风险，该项配置主要用于pixiv跨域图像抓取

### 更新日志

v0.3.0: 重构；使用纯js重写插件，指令更新为 `pixiv推荐作品`
