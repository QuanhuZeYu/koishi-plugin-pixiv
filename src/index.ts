...
export const Config: Schema<Config> = Schema.object({
	...
	HTMLSelector: Schema.object({
		...
		主图像查看全部按钮选择点击: Schema.string().description("如果你不知道这是什么请不要动它！！！这个选择器是尝试找查看全部按钮然后展开全部作品，方便获取所有图片").default("const buttonsDivs = Array.from(document.querySelectorAll(`main button div`));const lookAllDiv = buttonsDivs.find(div => div.textContent.includes(`查看全部`));lookAllDiv.click()"),
